<script lang="ts">
	...
	function draw() {
		...
		ctx.fillStyle = background.includes('#') ? background.split(' ')[0].replace('(', '') : '#0d0020';
		for (let i = 0; i < 8; i++) {
...
<style>
	.ng         { position: relative; overflow: hidden; }
	.ng-canvas  { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; }
