...
export type Theme = {
  ...
  label: string;
  description: string;
  // Visual axes (CSS classes at runtime / static vars in generated files)
  shape: ShapeVariant;
...
const SHAPE_RADII: Partial<Record<ShapeVariant, Record<string, string>>> = {
...
const SHADOWS: Partial<Record<ShadowVariant, Record<string, string>>> = {
...
const TYPO: Partial<Record<TypographyVariant, { weight: string; tracking: string; leading: string }>> = {
  tight: { weight: '800', tracking: '-0.04em', leading: '1.45' },
  relaxed: { weight: '600', tracking: '-0.01em', leading: '1.75' },
};

const FONTS: Partial<Record<FontVariant, { display: string; body?: string }>> = {
  mono: {
...
const TRANSITIONS: Partial<Record<TransitionVariant, { fast: string; base: string }>> = {
...
const TRACKING: Partial<Record<TrackingVariant, string>> = {
  wide: '0.04em',
  'tight-body': '-0.01em',
};

const TEXTURE_BG: Record<Exclude<TextureVariant, 'none'>, string> = {
  dots:
    'background-image: radial-gradient(circle, var(--texture-color, rgba(0, 0, 0, 0.055)) 1px, transparent 1px); background-size: 20px 20px;',
  lines:
    'background-image: repeating-linear-gradient(0deg, var(--texture-color, rgba(0, 0, 0, 0.04)) 0px, var(--texture-color, rgba(0, 0, 0, 0.04)) 1px, transparent 1px, transparent 28px);',
  grid:
    'background-image: linear-gradient(var(--texture-color, rgba(0, 0, 0, 0.05)) 1px, transparent 1px), linear-gradient(90deg, var(--texture-color, rgba(0, 0, 0, 0.05)) 1px, transparent 1px); background-size: 24px 24px;',
};
...
export function themeToCss(t: Theme): string {
  ...
  rootDecls.push(
    ...
    `--primary-hover: ${t.light.primaryHover};`,
    `--primary-fg: ${t.light.primaryFg};`,
    `--primary-subtle: ${t.light.primarySubtle};`,
    `--primary-subtle-fg: ${t.light.primarySubtleFg};`,
  );
  darkDecls.push(
    `--primary: ${t.dark.primary};`,
    `--primary-hover: ${t.dark.primaryHover};`,
    `--primary-fg: ${t.dark.primaryFg};`,
    `--primary-subtle: ${t.dark.primarySubtle};`,
    `--primary-subtle-fg: ${t.dark.primarySubtleFg};`,
  );

  // Background tints (optional)
  if (t.bgLight) {
    rootDecls.push(
      `--bg-base: ${t.bgLight.base};`,
      `--bg-subtle: ${t.bgLight.subtle};`,
      `--bg-muted: ${t.bgLight.muted};`,
      `--bg-hover: ${t.bgLight.hover};`,
    );
  }
  if (t.bgDark) {
    darkDecls.push(
      `--bg-base: ${t.bgDark.base};`,
      `--bg-subtle: ${t.bgDark.subtle};`,
      `--bg-muted: ${t.bgDark.muted};`,
      `--bg-hover: ${t.bgDark.hover};`,
    );
  }
  ...
  if (radii) for (const [k, v] of Object.entries(radii)) rootDecls.push(`${k}: ${v};`);

  // Shadow — mode-independent
  const shadow = SHADOWS[t.shadow];
  if (shadow) for (const [k, v] of Object.entries(shadow)) rootDecls.push(`${k}: ${v};`);

  // Font — sets display/body font-family variables
  const font = FONTS[t.font];
  if (font) {
    rootDecls.push(`--font-display: ${font.display};`);
    if (font.body) rootDecls.push(`--font-body: ${font.body};`);
  }

  // Transitions — mode-independent
  const tr = TRANSITIONS[t.transition];
  if (tr) rootDecls.push(`--transition-fast: ${tr.fast};`, `--transition-base: ${tr.base};`);

  // Tracking — body letter-spacing
  const tk = TRACKING[t.tracking];
  if (tk) rootDecls.push(`--tracking-body: ${tk};`);

  // Typography — heading tokens + concrete overrides on headings/body
  const typo = TYPO[t.typography];
  if (typo) {
    rootDecls.push(
      `--heading-weight: ${typo.weight};`,
      `--heading-tracking: ${typo.tracking};`,
      `--leading-base: ${typo.leading};`,
    );
    extraRules.push(
  ...
  if (font?.body) extraRules.push(`body {\n  font-family: var(--font-body);\n}\n`);
