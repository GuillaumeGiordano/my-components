...
function applyThemeClasses(t) {
    ...
    if (t.shape !== 'default')
        root.classList.add(`shape-${t.shape}`);
    if (t.shadow !== 'default')
        root.classList.add(`shadow-${t.shadow}`);
    if (t.typography !== 'default')
        root.classList.add(`type-${t.typography}`);
    if (t.font !== 'sans')
        root.classList.add(`font-${t.font}`);
    if (t.transition !== 'default')
        root.classList.add(`transition-${t.transition}`);
    if (t.texture !== 'none')
        root.classList.add(`texture-${t.texture}`);
    if (t.tracking !== 'default')
        root.classList.add(`tracking-${t.tracking}`);
}
// ── Cache localStorage (read by the inline script in app.html) ────────────
function saveThemeCache(t, m) {
    const classes = [];
    if (t.shape !== 'default')
        classes.push(`shape-${t.shape}`);
    if (t.shadow !== 'default')
        classes.push(`shadow-${t.shadow}`);
    if (t.typography !== 'default')
        classes.push(`type-${t.typography}`);
    if (t.font !== 'sans')
        classes.push(`font-${t.font}`);
    if (t.transition !== 'default')
        classes.push(`transition-${t.transition}`);
    if (t.texture !== 'none')
        classes.push(`texture-${t.texture}`);
    if (t.tracking !== 'default')
        classes.push(`tracking-${t.tracking}`);
    ...
    if (bg) {
        vars.push(['--bg-base', bg.base], ['--bg-subtle', bg.subtle], ['--bg-muted', bg.muted], ['--bg-hover', bg.hover]);
    }
    if (border) {
