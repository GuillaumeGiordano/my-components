<script lang="ts">
  ...
  const slotTop = (i: number) => PAD + i * (ITEM + GAP);
  ...
  const restTop = $derived(slotTop(activeIndex));
  const puckTop = $derived(dragging ? dragTop : restTop);
  // Item currently under the puck
  const targetIndex = $derived(
    dragging ? clampIndex(Math.round((dragTop - PAD) / (ITEM + GAP))) : activeIndex,
  );
  ...
  function onPuckMove(e: PointerEvent) {
    ...
    dragTop = Math.max(PAD, Math.min(slotTop(sections.length - 1), t));
  }

  function onPuckUp() {
    if (!dragging) return;
    const idx = clampIndex(Math.round((dragTop - PAD) / (ITEM + GAP)));
    dragging = false;
    go(sections[idx]);
  }
  ...
  function onPuckKey(e: KeyboardEvent) {
    if (e.key === "ArrowDown") {
      e.preventDefault();
      go(sections[clampIndex(activeIndex + 1)]);
    }
    if (e.key === "ArrowUp") {
      e.preventDefault();
      go(sections[clampIndex(activeIndex - 1)]);
