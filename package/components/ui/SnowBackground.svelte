<script lang="ts">
	...
	type Flake = { x: number; y: number; r: number; vx: number; vy: number; wobble: number; phase: number; opacity: number };
	...
	function draw() {
		...
		for (const f of flakes) {
			...
			if (f.y > H + 10) {
				const nf = mkFlake(false); f.x = nf.x; f.y = nf.y;
			}
			if (f.x > W + 10) f.x = -10;
			if (f.x < -10)    f.x = W + 10;
...
<style>
	...
	.sb-canvas  { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; }
