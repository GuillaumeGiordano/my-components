<script lang="ts">
	...
	type Grain = { x: number; y: number; r: number; vx: number; vy: number; opacity: number; color: string };
	...
	function draw() {
		...
		for (const g of grains) {
			...
			if (g.x < -g.r * 2) {
				const ng = mkGrain(false); g.x = ng.x; g.y = ng.y;
			}
...
<style>
	.sandb         { position: relative; overflow: hidden; }
	.sandb-canvas  { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; }
	.sandb-content { position: relative; z-index: 1; }
</style>
